...
try {
    ...
    // Hapus token yang lama (jika ada) dan buat token baru
    $newToken = rand(100000, 999999); // 6 digit token
    $expiresAt = date('Y-m-d H:i:s', strtotime('+1 hour')); // Token berlaku 1 jam

    // Simpan token baru ke database
    $stmt = $koneksi->prepare("INSERT INTO email_verification (email, token, expires_at) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $data['email'], $newToken, $expiresAt);

    if (!$stmt->execute()) {
        throw new Exception("Gagal mengirim token: " . $koneksi->error);
    }
